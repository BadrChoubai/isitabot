...
def interpret_result(prob: float) -> str:
    if prob < 0.5:
        return(f"I am {round((100*(1-prob)), 2)}% sure that that text was written by a bot.")
    else:
        return(f"I am {round((100*prob), 2)}% sure that that text was written by a human.")

# print(interpret_result(estimate_sample(single)))